...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	git clone --depth 1 --no-single-branch https://github.com/"$AUTHOR"/AnyKernel33.git
...
gen_zip() {
	...
	mv "$KERNEL_DIR"/out/arch/arm64/boot/$FILES AnyKernel33/$FILES
	if [ $BUILD_DTBO = 1 ]
	then
		mv "$KERNEL_DIR"/out/arch/arm64/boot/dtbo.img AnyKernel33/dtbo.img
	fi
	cdir AnyKernel3
