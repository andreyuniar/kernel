...
make_build(){
	...
	print "Make kernel dtb..." green	if [ $TOOLCHAIN == clang ]; then
...
gen_toolchain(){
	...
	git clone --depth=1 https://github.com/andreyuniar/AnyKernel33 -b master anykernel
...
build(){
	...
	then
		...
		make_build "${1}-surya_defconfig"

	fi

		mkzip "$KERNELNAME-Surya-OSS-$DATE.zip"
